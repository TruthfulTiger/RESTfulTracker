...
$(function () {
	sim1.attr('aria-hidden', 'true').hide();
	sim2.attr('aria-hidden', 'true').hide();
	sim3.attr('aria-hidden', 'true').hide();
	sim4.attr('aria-hidden', 'true').hide();
	sim5.attr('aria-hidden', 'true').hide();
	sim6.attr('aria-hidden', 'true').hide();
	sim7.attr('aria-hidden', 'true').hide();
	sim8.attr('aria-hidden', 'true').hide();
	checkEPs();
	...
	function facialHairRand() {
		...
		for (let i = 1; i <= numsims; i++) {
			let random_fhair = random_fhair = getWeightedRandom(list, weight);
			let ageTitle = $('#age' + i).attr("alt");

			if (random_fhair === 'nohair' || !ageTitle.includes("Adult") || !ageTitle.includes("Elder") || ageTitle.includes("Teen")) {
				fhimg = "turnoff10.png";
	...
	function prefRand() {
		...
		for (let i = 1; i <= numsims; i++) {
			...
			if (gender.prop("checked")) {
				...
				let genderTitle = $('#gender' + i).attr("title");

				if (genderTitle.includes("Female") && random_pref === "straight" ||
					genderTitle.includes("Male") && random_pref === "gay") {
					prefimg = "pref_male.png";
					prefalt = "Prefers males";
				}
				if (genderTitle.includes("Female") && random_pref === "gay" ||
					genderTitle.includes("Male") && random_pref === "straight") {
	...
	function displayResult() {
		...
		if (facialHair.prop("checked")) {
			...
			for (let i = 1; i <= numsims; i++) {
				let genderTitle = $('#gender' + i).attr('title');
				if (gtest.test(genderTitle)) {
					$('#facialHair' + i).attr("src", s2path + "turnoff10.png");
					$('#facialHair' + i).attr("alt", "Has no facial hair");
					$('#facialHair' + i).attr("title", "Has no facial hair");
				}
			}
		} else {
			for (let i = 1; i <= numsims; i++) {
...
function randomSims() {
	...
	numberSims.val(random);
}

function checkSims(n) {
	if (n == 1) {
		sim1.attr('aria-hidden', 'false').show();
		sim2.attr('aria-hidden', 'true').hide();
		sim3.attr('aria-hidden', 'true').hide();
		sim4.attr('aria-hidden', 'true').hide();
		sim5.attr('aria-hidden', 'true').hide();
	...
	if (n == 2) {
		sim1.attr('aria-hidden', 'false').show();
		sim2.attr('aria-hidden', 'false').show();
		sim3.attr('aria-hidden', 'true').hide();
		sim4.attr('aria-hidden', 'true').hide();
		sim5.attr('aria-hidden', 'true').hide();
		sim6.attr('aria-hidden', 'true').hide();
	...
	if (n == 3) {
		sim1.attr('aria-hidden', 'false').show();
		sim2.attr('aria-hidden', 'false').show();
		sim3.attr('aria-hidden', 'false').show();
		sim4.attr('aria-hidden', 'true').hide();
		sim5.attr('aria-hidden', 'true').hide();
		sim6.attr('aria-hidden', 'true').hide();
		sim7.attr('aria-hidden', 'true').hide();
	...
	if (n == 4) {
		...
		sim2.attr('aria-hidden', 'false').show();
		sim3.attr('aria-hidden', 'false').show();
		sim4.attr('aria-hidden', 'false').show();
		sim5.attr('aria-hidden', 'true').hide();
		sim6.attr('aria-hidden', 'true').hide();
		sim7.attr('aria-hidden', 'true').hide();
		sim8.attr('aria-hidden', 'true').hide();
	...
	if (n == 5) {
		...
		sim3.attr('aria-hidden', 'false').show();
		sim4.attr('aria-hidden', 'false').show();
		sim5.attr('aria-hidden', 'false').show();
		sim6.attr('aria-hidden', 'true').hide();
		sim7.attr('aria-hidden', 'true').hide();
		sim8.attr('aria-hidden', 'true').hide();
	}
	...
	if (n == 6) {
		...
		sim4.attr('aria-hidden', 'false').show();
		sim5.attr('aria-hidden', 'false').show();
		sim6.attr('aria-hidden', 'false').show();
		sim7.attr('aria-hidden', 'true').hide();
		sim8.attr('aria-hidden', 'true').hide();
	}
	...
	if (n == 7) {
		...
		sim5.attr('aria-hidden', 'false').show();
		sim6.attr('aria-hidden', 'false').show();
		sim7.attr('aria-hidden', 'false').show();
		sim8.attr('aria-hidden', 'true').hide();
	}
	...
	if (n == 8) {
		...
		sim6.attr('aria-hidden', 'false').show();
		sim7.attr('aria-hidden', 'false').show();
		sim8.attr('aria-hidden', 'false').show();
	}
}